...
class Classifier_init(nn.Module):
    def __init__(self, inputDimV, hiddenDimV, birnnV, d_word_emb, outdpV, hiddenDNNV,
        ...
        self.outProj_L0 = nn.Linear(
            in_features=Nh * hiddenDimV, out_features=hiddenDNNV)
        ...
        self.outProj_L1 = nn.Linear(
            in_features=hiddenDNNV, out_features=hiddenDNNV // 2)
        self.outDO_L1 = nn.Dropout(p=outdpV)
        self.outProj_L2 = nn.Linear(
            in_features=hiddenDNNV // 2, out_features=1)
        self.indpV = 0.20
        self.g2p = g2p
        if self.g2p:
            self.enc_dec = G2P(
        ...
        if self.shortcut == True:
            self.conv3 = nn.Conv2d(1, 32, kernel_size=5, stride=(
                1, 1), padding=(2, 2))  # padding used to (2,2)
            self.conv1 = nn.Conv2d(64, 128, kernel_size=5, stride=(
                2, 2), padding=(2, 2))  # padding used to (2,2)
            self.conv2 = nn.Conv2d(128, 256, kernel_size=5, stride=(
                1, 2), padding=(2, 2))  # padding used to be (2,2)
            ...
            self.conv1 = nn.Conv2d(1, 32, kernel_size=5, stride=(
                2, 2), padding=(2, 2))  # padding used to (2,2)
            self.conv2 = nn.Conv2d(32, 64, kernel_size=5, stride=(
                1, 2), padding=(2, 2))  # padding used to be (2,2)
            self.batch1 = nn.BatchNorm2d(32)
            self.max_pool = nn.MaxPool2d(2, stride=(1, 2))
    ...
    def forward(self, x, x_lengths, p_lengths, grapheme, phoneme):
        ...
        if self.fixed_length_embedding == True:
            ...
            word_embedding = word_embedding.unsqueeze(1).expand(batch_size, x.data.size(
                1), self.d_word_emb)  # nb videos x number of frames x 128
        else:
            word_embedding = self.wBN(
                all_emb.transpose(-2, -1)).transpose(-2, -1)

        if self.loc_acc:
            x = self.inBN(x.reshape(-1, self.inputDimV)
                          ).view(batch_size, -1, self.inputDimV)
        else:

            x = self.inBN(x.view(-1, self.inputDimV)
                          ).view(batch_size, -1, self.inputDimV)
        if self.training:
        ...
        if self.rnn2_present == False:
            ...
            attention_mask = attention(
                query, key, value, batch_size, nb_videos, self.num_heads, self.d_k)
        ...
        if self.rnn2_present == True:
            ...
            pack = torch.nn.utils.rnn.pack_padded_sequence(
                attention_mask, x_lengths, batch_first=True)
            o, _ = self.rnn2(pack)
            o, _ = torch.nn.utils.rnn.pad_packed_sequence(o, batch_first=True)
            ...
            if self.shortcut:
                ...
                o = self.batch1(o)
                o = F.relu(o)
                shortcut = self.project_query(
                    query).transpose(-2, -1).unsqueeze(-2).expand(-1, -1, nb_videos, -1)
                ...
                o = self.conv1(
                    attention_mask.transpose(-1, -3).transpose(-1, -2))
                o = self.batch1(o)
                o = F.relu(o)
                o = self.max_pool(o)
                o = self.conv2(o)
...
def attention(query, key, value, batch_size, nb_v_frames, num_heads, d_k):
    if len(value) == 0:
        ...
        attention_mask = torch.matmul(attention_mask, value).view(
            batch_size, len_p, num_heads*d_k)
    ...
    def _generate(self, emb, context):
        ...
        if context is not None:
            context = context.expand(
                beam.size, context.size(1), context.size(2))
...
class Encoder(BaseModule):
    ...
    def forward(self, x_seq, cuda=True):
        ...
        all_cell_states = cell_state.unsqueeze(
            -1).transpose(-2, -1).repeat(1, nb_phonemes, 1)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--birnnV', type=int, default=1,
                        help="use bidirectional lstms")
    ...
    parser.add_argument("--g_size", type=int, default=800,
                        help="size of grapheme vocab")
    parser.add_argument("--p_size", type=int, default=510,
                        help="size of phoneme vocab")
